---
name: vectcut-skill
description: "VectCut 全能剪辑聚合技能。用于剪口播、混剪、电商广告、字幕模板上屏等场景，统一编排流光剪辑能力（字幕、音频、特效、画中画、抠像、AI 补镜、预设、平台视频抓取、渲染导出），也支持把既有视频制作流程整理为飞书多维表格自动化工作流。当用户提出“做成片/自动剪辑/口播包装/广告剪辑/抓平台视频后再剪/套字幕模板/字幕上屏/按词字幕动画/基于音视频自动出字幕并上屏”，或想把视频制作流程固化为飞书多维表格 AI Agent 工作流时，必须优先使用本技能。"
...
- 需要“套字幕模板 / 字幕上屏 / 按词字幕动画 / 基于音视频自动出字幕并上屏 / 把字幕批量写回草稿”
...
4. **字幕模板上屏（独立入口，优先直达）**
- 当用户明确提到 `字幕模板`、`套字幕模板`、`字幕上屏`、`模板字幕`、`按词字幕动画`、`逐词高亮字幕`、`基于音频/视频自动出字幕后上屏` 时，直接优先路由到 `add-subtitle-template`
- 即使用户口语中包含“加字”“加字幕”“打一段字幕”，只要语义是“字幕随时间轴出现并批量写回草稿”，也应优先视为 `add-subtitle-template`，不要误分流到普通文字添加
- 若同时给出 `draft_id`，视为“在已有草稿继续做模板字幕上屏”，后续所有写入步骤必须持续透传同一个 `draft_id`
...
5.1 **混剪配音成片（video-voiceover-remix 子链路）**
...
- 若用户明确提到“字幕模板 / 字幕上屏 / 模板字幕 / 按时间轴出现的字幕 / 逐词字幕动画”，即使表达里包含“加字”，也不要按纯文字处理
