...
1. 当用户要生成儿童绘本网页时，优先参考 `references/index.html` 的结构、字段命名、步骤拆分、接口调用方式和展示区布局。
2. 当用户要修改已有网页时，优先沿用参考实现和现有实现中的命名习惯、状态流转、字段提取逻辑，按最小 Diff 修改。
3. 当用户问“具体逻辑/原理/为什么失败”时，优先从 `references/index.html` 或用户当前提供的 `index.html` 中提取真实代码逻辑来回答，不要只给抽象解释。
...
- 如存在参考实现，优先吸收 `references/index.html`
...
- 优先阅读当前真实文件或 `references/index.html`
- 回答时以真实实现为依据
- 先给结论，再给依据，再给建议
