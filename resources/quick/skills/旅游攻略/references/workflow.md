...
8. 对每个口播源或纯素材目标时间轴调用一次 LLM Chat 规划，结合字幕时间轴、图片描述和视频理解结果严格解析 `plans` 并校验覆盖、索引和时间；口播模式始终以字幕时间轴为主轴，让图片和视频内容向主轴靠拢。解析后只做轻量边界校正，例如把最后一段补齐到总时长、把相邻毫秒级边界对齐；如果存在非法素材索引、负时长、明显空洞/重叠、空搜索词或短到无法展示的片段，进入 LLM 重试。
...
12. 按计划添加视频、图片和图片缩放关键帧。添加连续视频片段时先为每个片段边界判定转场，并把转场字段写入前一个视频片段的 `add_video` 请求：前后无明确关系用 `翻页`，其余用 `左移`，默认 `transition_duration=0.5`；最后一个视频片段不加转场。
...
- LLM 规划解析或时间轴校验失败时，使用同一输入完整重试一次；第二次仍失败时停止。不要为了继续生成而写死素材索引顺序或套用固定分镜。
