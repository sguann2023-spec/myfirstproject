...
5. 识别与清洗字幕。分三个子步骤，每步结果分别保存为独立文件，所有文件写入**当前工作空间根目录**：
   ...
   2. **分句清洗 → 文件 2**。分析文件 1，按**语义完整性**拆分分句（在主谓宾、动补、连词等自然语义边界处断开），而非按固定字数硬切割；每句不超过 12 个字，超过时在下一个语义断点处拆分。每条分句**结尾不得包含任何标点符号**（逗号、句号、问号、叹号、顿号等全部去除）。然后对分句执行：去气口（删除纯语气词/停顿片段）、去重复（移除完全重复的句子）、去错别字（修正明显识别错误）、去除需要重录的片段（标记为无效或不可用的片段）。清洗后的结果保存为当前工作空间下的第二个文件（如 `{workspace}/asr_cleaned_sentences.json`）。
...
10. 组装并执行工作流。把添加主视频片段、标题、双语字幕、关键词弹出、转场、缩放关键帧、提示音和 BGM 组装成 workflow（不包含 `create_draft` 步骤），在 workflow 的 `inputs` 中传入第 9 步获取的 `draft_id`，先保存workflow的请求内容为文件，再调用工作流执行工具，减少串行接口调用。本地视频无需上传为临时链接，工作流中直接使用本地文件路径即可。
