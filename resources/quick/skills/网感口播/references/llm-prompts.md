...
- **每行严格不超过 8 个中文、英文或数字字符（硬限制，不可违反）。** 生成后必须逐字符计数验证，超过 8 个字符的标题必须截断或重新措辞。
- 不要标点符号、空格或装饰符号。
- 如果原文核心内容无法用两行 8 字符以内概括，优先截取最有冲击力的关键词，不要求语义完整。
- 兜底方案：用 ASR 前 16 个有效字符按 8+8 截成一到两行。

### 标题字数正确示例

- ✅ `AI不需要海量` (7字符) + `需要重度用户` (6字符)
- ✅ `别再卷用户量` (7字符) + `重度用户才关键` (7字符)
- ✅ `知识付费凉了` (6字符) + `AI时代变了` (6字符)

### 标题字数错误示例

- ❌ `AI时代不需要海量用户` (11字符) — 超过 8 字符限制
- ❌ `未来商业模式核心逻辑` (10字符) — 超过 8 字符限制
...
**单条字幕最多按 12 个有效字符规划（硬限制，不可违反）。** 有效字符按去标点后的文字字符计数，中文、英文、数字均逐字符计入。任何一条字幕超过 12 个有效字符即为不合格输出，必须重新拆分。

- 有效字符 `1-7` 个：普通单行字幕，`layered=false`，只填写 `normal_text/normal_en`。
- 有效字符 `8-12` 个：上下分层字幕，`layered=true`，填写 `top_text/bottom_text/top_en/bottom_en`，每行尽量不超过 `8` 个有效字符。
- 有效字符大于 `12` 个：必须在同一句 ASR 句子内部拆分成多条字幕，每段尽量控制在 `8-12` 个有效字符；严禁把超长句当作一条字幕输出，也严禁把分层上下两行合计当作超长字幕的借口。
- 分层字幕上下两行拼回必须等于当前字幕文本，单行尽量不超过 `8` 个有效字符。
- 生成后必须逐条自检有效字符数；超过 12 字的字幕必须回到当前 ASR 句内部重新拆分后再输出。
- **分层切词硬规则**：上下分层的切分点必须落在词与词之间，严禁把同一个词拆到两行。具体来说：
  - 专有名词（如"知识付费""蒙太奇""GitHub""Web Workflow""Agent""Skills""AI"）必须保持完整，不能从中间断开。
  - 常用搭配（如"做工具""不挣钱""实拍""工作流""画布工具"）不能拆开。
  - 代词+动词（如"我说""你觉得""他知道"）不能拆开。
  - 如果找不到合适的切分点，宁可让上行多承担几个字，也不要从词中间切。
  - 校验时如果发现上下行切分点落在一个词中间，必须重新选择切分位置。
...
- 关键词弹出采用「底层挖空 + 覆盖层弹出」两层叠加机制，底层与覆盖层必须字符数完全一致、逐位对齐：
  - **底层字幕**（`display_text`）：显示完整字幕文案，但把被选中弹出的关键词字符逐字替换为全角空格 `\u3000`，其余文字保持不变。底层只负责展示普通文案，关键词位置留空。
  - **覆盖层**（`keyword_pop_text`）：只在关键词位置显示真实关键词字符，其余所有位置用全角空格 `\u3000` 占位。覆盖层字符长度必须与底层完全相同，保证两层叠加时关键词恰好落在底层留空的位置上。
...
- **关键词弹出层样式（必须全部设置，禁止遗漏）**：`intro_animation=左移弹动`、`font_size=15`、`font_color=#A81C23`、`shadow_enabled=true`、`shadow_color=#ffffff`、上行 `intro_duration=0.16`、普通/下行 `intro_duration=0.25`。
- 只要规划了 `keyword_pop_text`（非空），后处理时必须创建关键词弹出层，与是否命中 `打字机_I` 动画无关。未规划关键词的字幕不创建弹出层。
...
1.5. **标题字数硬校验**：逐字符计数 `top_title` 和 `bottom_title`，任何一行超过 8 个字符时必须截断到 8 个字符或重新措辞。截断优先级：保留最有冲击力的关键词，不保留虚词（的、了、是、在、和）。如果截断后语义不通，用 ASR 原文前 8 个字符兜底。校验不通过时不允许跳过，必须修正后再继续。
1.6. **字幕字数硬校验**：逐条统计 `subtitle_items` 的有效字符数（`layered=true` 时统计上下两行拼回后的总数，`layered=false` 时统计 `normal_text`）。任何一条超过 12 个有效字符的，必须回到当前 ASR 句内部重新拆分成多条 `8-12` 字的字幕；同时检查 1–7 字的条目是否误用了分层、8–12 字的条目是否误用了普通单行。校验不通过时不允许跳过，必须修正后再继续。
2. `subtitle_items` 必须是数组；对象字段缺失时按空值补齐。
3. 按 `source_index` 分组，检查每组字幕拼回后等于对应 ASR 句去标点文本。
4. 如果长句拆分失败，按每 `12` 个有效字符切块（单条上限，不可放宽到更多字），并按字符数比例分配当前 ASR 句时间。
5. 如果分层上下行拼不回原文，或单行过长，用本地语义切分兜底。
6. **分层切词校验**：检查每条分层字幕的上下行切分点是否落在词与词之间。如果发现专有名词（如"知识付费""AI""GitHub""Agent""Skills""Web Workflow""蒙太奇"）、常用搭配或代词+动词被从中间拆开，必须重新选择切分位置，宁可让某一行多承担几个字也不能切词。
7. 关键词只保留真实出现在当前字幕文本里的词；弹出关键词只取当前字幕里最后出现的一个。
...
**⚠️ 位置参数硬约束**：组装工作流时，每层字幕的 `transform_x_px`、`transform_y_px`、`align`、`fixed_width` 必须严格复制 `workflow.md` 中「字幕层位置硬约束」表格里定义的精确数值。禁止自行估算、推算或编造坐标。关键词弹出层必须设置 `intro_animation=左移弹动`、`font_size=15`、`shadow_enabled=true`、`shadow_color=#ffffff`，上行弹出时长 `0.16`，普通/下行弹出时长 `0.25`。
