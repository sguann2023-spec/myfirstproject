#!/usr/bin/env python3
"""Recognize narration subtitles for the talking-head selling mixed-edit skill."""
...
try:
    ...
    from timeline import build_timeline, extract_segments, remap_segments, validate_timeline
except ImportError:  # pragma: no cover
    from .vectcut_api import VectCutClient, VectCutError, is_asr_complete, poll, task_id
    from .timeline import build_timeline, extract_segments, remap_segments, validate_timeline


DEFAULT_EFFECT_MODE = "llm_vad"
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Recognize ASR subtitles for talking-head selling narration")
    ...
    parser.add_argument("--source-duration", type=float, help="source video duration in seconds; when supplied, also emit the compact source-to-target timeline")
...
def recognize(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    submitted = client.submit_asr(args.url, effect_mode=args.effect_mode, content=content)
    ...
    if args.source_duration is not None:
        timeline_segments = extract_segments(result)
        ...
        timeline["segments"] = remap_segments(timeline_segments, timeline)
