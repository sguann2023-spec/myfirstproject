...
本文件描述口播卖货混剪里的音频写入步骤。执行时优先使用 `scripts/hunjian_task.py add-audio`。
...
- `target_start`：草稿时间轴里的目标开始时间，单位秒；去气口场景必须使用 `timeline.json` 对应 chunk 的目标起点，不能默认整段从 `0` 开始。
...
- `volume`：`20`
- `target_start`：来自 `timeline.json`；只有没有删除任何源时间时才使用 `0` 的整段音频。
- `speed`：`1.0`

去气口场景必须循环写入，与视频轨完全一致：
...
`start/end` 是源音频区间，`target_start` 是压缩后草稿区间。视频 chunk 和音频 chunk 必须使用同一个映射；如果 `removed_duration_ms > 100`，禁止用一条 `start=0/end=duration/target_start=0` 的音频替代。
