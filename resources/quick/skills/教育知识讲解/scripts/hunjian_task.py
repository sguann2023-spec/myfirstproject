...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="VectCut API wrapper for the home decoration mixed-edit skill")
    ...
    for name, help_text in (("asr", "submit or poll ASR"), ("llm", "submit or poll LLM"), ("video-detail", "submit or poll video analysis"), ("video-capture", "submit or poll semantic video capture")):
    ...
    p = sub.add_parser("add-text-template", help="add a home decoration text template")
    p.add_argument("--payload-json", required=True, help="full add_text_template request JSON")

    p = sub.add_parser("add-text", help="add a text or flower text segment")
    p.add_argument("--payload-json", required=True, help="full add_text request JSON")

    p = sub.add_parser("search-sticker", help="search a sticker by keywords")
    p.add_argument("--payload-json", required=True, help="full search_sticker request JSON")

    p = sub.add_parser("add-sticker", help="add a sticker segment")
    p.add_argument("--payload-json", required=True, help="full add_sticker request JSON")

    p = sub.add_parser("add-preset", help="add a sound effect preset")
    p.add_argument("--payload-json", required=True, help="full add_preset request JSON")

    sub.add_parser("list-scene-effects", help="list supported scene effect names")

    sub.add_parser("list-character-effects", help="list supported character effect names")

    p = sub.add_parser("add-effect", help="add a scene or character effect")
    p.add_argument("--payload-json", required=True, help="full add_effect request JSON")
    return parser
...
def _run(args: argparse.Namespace, client: VectCutClient) -> Dict[str, Any]:
    ...
    if args.command == "search-sticker":
        return client.search_sticker(_json_arg(args.payload_json, "--payload-json"))
    if args.command == "add-sticker":
        return client.add_sticker(_json_arg(args.payload_json, "--payload-json"))
    if args.command == "add-preset":
        return client.add_preset(_json_arg(args.payload_json, "--payload-json"))
    if args.command == "list-scene-effects":
        return client.get_video_scene_effect_types()
    if args.command == "list-character-effects":
        return client.get_video_character_effect_types()
    if args.command == "add-effect":
        return client.add_effect(_json_arg(args.payload_json, "--payload-json"))
