...
- 每条字幕从 `无动画`、`渐显`、`打字机_III` 三档中随机选择。
- 整体比例保持近似各三分之一；字幕数量能被 3 整除时三档数量完全一致。
- 无动画传 `intro_animation=""`、`intro_duration=0.0`。
- `渐显` 传 `intro_animation="渐显"`、`intro_duration=0.35`。
- `打字机_III` 传 `intro_animation="打字机_III"`、`intro_duration=0.45`。
...
- 排比预设接管的 `text1` 不写入 `manual_subtitle`；若它位于较长句子中间，按 ASR 词级首尾时间拆成短语前、短语后两条普通字幕。
