...
def build_original_duration_timeline(segments: list[dict[str, Any]], source_duration_ms: int) -> dict[str, Any]:
    """Keep every source frame, including pauses and trailing silence."""
    if source_duration_ms <= 0:
        raise ValueError("source_duration_ms must be positive")
    return {
        "source_duration_ms": source_duration_ms,
        ...
        "chunks": [
            {
                ...
                "target_start_ms": 0,
                "target_end_ms": source_duration_ms,
...
def _main() -> int:
    ...
    source_duration_ms = int(round(args.source_duration * 1000))
    timeline = (
        build_original_duration_timeline(segments, source_duration_ms)
        if args.keep_original_duration
        else build_timeline(
            segments,
            source_duration_ms,
