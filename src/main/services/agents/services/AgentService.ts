...
export class AgentService extends BaseService {
  ...
  /**
   * Initialize the built-in VectcutClaw agent with a fixed ID.
   * Called once at app startup. Safe to call multiple times — skips if the agent already exists.
   * Returns the agent ID if created or already present, or null if no compatible model is available yet.
   */
  async initDefaultCherryClawAgent(): Promise<BuiltinAgentInitResult> {
    ...
    try {
      ...
      if (existing?.deleted_at) {
        logger.info('Default VectcutClaw agent was deleted by user — skipping recreation', { id })
        return { agentId: null, skippedReason: 'deleted' }
      }
      ...
      const firstModel = modelsRes.data?.[0]
      if (!firstModel) {
        logger.info('No Anthropic-compatible models available yet — skipping default VectcutClaw creation')
        return { agentId: null, skippedReason: 'no_model' }
      }
      ...
      const req: CreateAgentRequest = {
        type: 'claude-code',
        name: 'Cherry Claw',
        description: 'Default autonomous VectcutClaw agent',
        model: firstModel.id,
        accessible_paths: [],
        configuration
      }
      ...
      try {
        ...
        logger.warn('Failed to seed builtin skills for VectcutClaw agent', {
          agentId: id,
          error: error instanceof Error ? error.message : String(error)
        })
      }

      logger.info('Created default VectcutClaw agent', { id })
      return { agentId: id }
    } catch (error) {
      logger.error('Failed to init default VectcutClaw agent', error as Error)
      return { agentId: null, skippedReason: 'no_model' }
    }
  }
