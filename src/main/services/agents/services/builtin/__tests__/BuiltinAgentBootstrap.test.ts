...
describe('bootstrapBuiltinAgents', () => {
  ...
  it('retries built-in bootstrap when no model is available yet', async () => {
    mockInitDefaultCherryClawAgent
      ...
      .mockResolvedValueOnce({ agentId: 'vectcut_claw_default' })
    ...
    expect(mockListSessions).toHaveBeenCalledWith('vectcut_claw_default', { limit: 1 })
    expect(mockCreateSession).toHaveBeenCalledWith('vectcut_claw_default', {})
    expect(mockEnsureHeartbeatTask).toHaveBeenCalledWith('vectcut_claw_default', 30)
  })
