 ...
 * (VectcutClaw, Cherry Assistant, etc.). Keeps business details out of
...
async function initCherryClaw(): Promise<void> {
  try {
    ...
    await handleInitResult(CHERRY_CLAW_AGENT_ID, 'VectcutClaw', result, initCherryClaw, async (agentId) => {
      await schedulerService.ensureHeartbeatTask(agentId, 30)
    })
  } catch (error) {
    logger.warn('Failed to init VectcutClaw agent:', error as Error)
  }
}
