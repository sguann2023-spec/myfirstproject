...
describe('AgentService built-in agent lifecycle', () => {
  ...
  it('skips recreating a built-in agent that was soft-deleted by the user', async () => {
    const database = {
      select: vi.fn(() =>
        createSelectQuery([{ id: 'vectcut_assistant_default', deleted_at: '2026-04-15T00:00:00.000Z' }])
      )
    }
  ...
  it('soft-deletes built-in agents while preserving the row', async () => {
    ...
    const database = {
      select: vi.fn(() => createSelectQuery([{ id: 'vectcut_claw_default', deleted_at: null }])),
    ...
    const deleted = await service.deleteAgent('vectcut_claw_default')
