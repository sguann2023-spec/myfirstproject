...
describe('PromptBuilder', () => {
  ...
  describe('buildToolGuidance', () => {
    ...
    it('adds only capability-scoped guidance', () => {
      ...
      expect(result).toContain('prefer Git Bash-compatible commands and POSIX shell syntax')
      expect(result).toContain('avoid suggesting or emitting PowerShell- or cmd.exe-specific syntax')
      ...
      expect(result).toContain('avoid emitting multiple large write payloads in a single assistant message')
      expect(result).toContain('Prefer sequential writes and smaller steps over parallel large writes')
      expect(result).toContain('prefer local scripts or shell heredocs')
