...
class SocialCopywritingServer {
  ...
  private setupHandlers() {
    ...
    this.mcpServer.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        switch (toolName) {
          case 'extract_social_copywriting_prompt':
            return await this.extractSocialCopywritingPrompt(args as Record<string, unknown>)
          default:
  ...
  private async waitForAsrResult(taskId: string) {
  ...
  private async analyzeTranscript(
    ...
    const response = await this.requestJson(CHAT_ENDPOINT, {
      method: 'POST',
      body: {
    ...
    if (!content) {
      throw new Error(`Chat analysis returned empty content: ${JSON.stringify(payload)}`)
    }

    return {
      model: String(payload.model || analysisModel || 'gpt-5.5'),
      raw: content,
  ...
  private async extractSocialCopywritingPrompt(args: Record<string, unknown>) {
    ...
    const artifacts = await this.createTempArtifacts(videoUrl)
    ...
    try {
      ...
      const asrStatus = await this.waitForAsrResult(String(asrSubmit.task_id))
      const transcript = String(asrStatus.result?.content || '').trim()
      if (!transcript) {
        throw new Error('ASR completed but result.content is empty')
      }

      const analysis = await this.analyzeTranscript(transcript, parsed.data, topic, analysisModel)
      const resultPayload = {
        provider: 'vectcut',
        action: 'extract_social_copywriting_prompt',
        request: {
          platform,
          shareText,
          topic,
