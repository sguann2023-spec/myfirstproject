...
export const loadTopicMessagesThunk =
  ...
  async (dispatch: AppDispatch, getState: () => RootState) => {
    ...
    try {
      ...
      const blockTypeCounts = blocks.reduce<Record<string, number>>((acc: Record<string, number>, block: MessageBlock) => {
        const key = String(block?.type || 'unknown')
        acc[key] = (acc[key] || 0) + 1
        return acc
      }, {})
      const toolBlocks = blocks.filter((block: MessageBlock) => block?.type === MessageBlockType.TOOL)
      const skillToolBlocks = toolBlocks.filter(
        (block: MessageBlock) =>
          block?.metadata?.rawMcpToolResponse?.tool?.name === 'Skill' ||
          block?.metadata?.rawMcpToolResponse?.tool?.id === 'Skill'
      )
      ...
      logger.silly('Loaded messages via DbService', {
        ...
        skillToolBlockCount: skillToolBlocks.length
      })
