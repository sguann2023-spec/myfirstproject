...
const isAnthropicMessageStream = (value: unknown): value is MessageStream => {
  if (!value || typeof value !== 'object') return false
  const candidate = value as Record<string, unknown>
  return typeof candidate.on === 'function' && typeof candidate.finalMessage === 'function'
}
    ...
    if (result instanceof Promise) {
      return result
        .then((data) => {
          ...
          } else if (isAnthropicMessageStream(data)) {
