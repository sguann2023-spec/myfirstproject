...
MODEL_CONFIG_JSON = {
    "models": [
        {"name": "gemini-3.1-pro-preview", "project_id": 16, "provider_id": "gemini", "provider_type": "gemini", "provider_name": "Gemini"},
        {"name": "gemini-3.1-flash-lite-preview", "project_id": 63, "provider_id": "gemini", "provider_type": "gemini", "provider_name": "Gemini"},
        {"name": "qwen3.6-plus", "project_id": 49, "provider_id": "openai", "provider_type": "openai", "provider_name": "OpenAI"},
        {"name": "claude-opus-4-7", "project_id": 58, "provider_id": "anthropic", "provider_type": "anthropic", "provider_name": "Anthropic"},
        {"name": "gpt-5.5", "project_id": 65, "provider_id": "openai", "provider_type": "openai", "provider_name": "OpenAI"},
    ],
    ...
    if btype == "image_url":
        ...
        if isinstance(image_url, dict):
            ...
            if isinstance(url2, str) and url2:
                return {"type": "image_url", "image_url": {"url": url2}}
        return None
    return None
...
def _anthropic_content_to_openai_content(content):
    ...
    if not items:
        return ""
    if all(isinstance(it, dict) and it.get("type") == "text" for it in items):
...
def _openai_message_to_anthropic_content(message_obj):
    blocks = []
...
def _build_anthropic_non_stream_response(openai_json: dict, fallback_model: str):
    ...
    return {
        ...
        "role": "assistant",
        "model": model_name,
        "content": _openai_message_to_anthropic_content(message_obj if isinstance(message_obj, dict) else {}),
...
def _adapt_messages_for_model(messages: list, is_qwen_model: bool):
    adapted = []
...
def _api_key_for_model(model_name: str):
    if isinstance(model_name, str) and model_name.startswith("qwen"):
        return QWEN_API_KEY
...
def _stream_chat_with_retry(
    ...
    for idx, attempt_model in enumerate(candidate_models):
        ...
        is_qwen_model = isinstance(attempt_model, str) and attempt_model.startswith("qwen")
        attempt_messages = _adapt_messages_for_model(messages, is_qwen_model)
        if is_qwen_model and enforce_json and not _contains_json_word(attempt_messages):
            has_system = False
            for msg in attempt_messages:
                if isinstance(msg, dict) and msg.get("role") == "system":
                    ...
                    if isinstance(c, str):
                        ...
                        msg["content"] = (c + [{"type": "text", "text": "Please return valid JSON only."}])
                    else:
                        msg["content"] = "Please return valid JSON only."
                    has_system = True
                    break
            if not has_system:
                attempt_messages.insert(0, {"role": "system", "content": "Please return valid JSON only."})
        host = QWEN_API_HOST if is_qwen_model else API_HOST
        path = QWEN_API_PATH if is_qwen_model else API_PATH
        api_key = _api_key_for_model(attempt_model)
        upstream_body = {"model": attempt_model, "messages": attempt_messages, "stream": True}
        ...
        if is_qwen_model:
            thinking_enabled = False if enable_thinking is None else bool(enable_thinking)
            ...
            if not (isinstance(attempt_model, str) and attempt_model.startswith("gpt")):
                _apply_thinking_instruction(attempt_messages, is_qwen_model=False, enable_thinking=bool(enable_thinking))
        ...
        try:
            ...
            if not client_stream:
                ...
                final_resp = {
                    ...
                    "choices": [{"finish_reason": finish_reason, "index": 0, "message": {"content": assistant_content, "role": "assistant"}}],
                    "usage": usage_obj
                }
                ...
                if isinstance(user_id, str) and user_id and isinstance(session_id, str) and session_id:
                    add_message(user_id, session_id, "assistant", assistant_content, model=latest_model, response_code=res.status, meta=final_resp)
            ...
            def generate():
                ...
                try:
                    ...
                    if isinstance(user_id, str) and user_id and isinstance(session_id, str) and session_id:
                        assistant_content = "".join(assistant_parts)
                        assistant_std = {
                            "choices": [{"finish_reason": "stop", "index": 0, "message": {"content": assistant_content, "role": "assistant"}}],
                        ...
                        add_message(user_id, session_id, "assistant", assistant_content, model=attempt_model, response_code=res.status, meta=assistant_std)
...
def _chat_impl(force_stream: Optional[bool] = None):
    ...
    is_qwen_model = isinstance(model, str) and model.startswith("qwen")
    messages = []
